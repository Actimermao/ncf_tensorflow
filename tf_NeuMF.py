...
def evaluate_one_rating(idx):
    ...
    ranklist = heapq.nlargest(topk, map_score, key=map_score.get)
...
if __name__ == '__main__':
    ...
    mlp_embedding_user = tf.Variable(tf.random_normal(shape=[num_users, int(layers[0]/2)], mean=0.0, stddev=0.01))
    mlp_embedding_item = tf.Variable(tf.random_normal(shape=[num_items, int(layers[0]/2)], mean=0.0, stddev=0.01))
    gmf_embedding_user = tf.Variable(tf.random_normal(shape=[num_users, num_factors], mean=0.0, stddev=0.01))
    gmf_embedding_item = tf.Variable(tf.random_normal(shape=[num_items, num_factors], mean=0.0, stddev=0.01))
    ...
    mlp_dense = hidden[-1]
    neu_dense = tf.concat([gmf_dense, mlp_dense], 1)
    w_neu = tf.Variable(tf.random_normal(shape=[2*num_factors, 1], mean=0.0, stddev=0.01))
